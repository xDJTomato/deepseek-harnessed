...
- **禁止委派清单写进桥接层**:`dsh_task` 描述新增【这些活不要派给子代理】一段,`initialize.instructions`
  第 7 条新增第 ③ 项。四类活不派:① **短指令作业**(一条命令、一次只读查询、改一行、看一个文件、
  跑一次 `git status` —— 预计 < 30s 且不需要独立上下文的,自己做,派出反而更慢);② **常驻/长命进程**
  (dev server、watch、daemon、长驻或交互式终端);③ **不能轻易结束的进程**(占端口、开迁移/事务、拿锁、
  必须成对收尾的独占资源);④ 需要人交互的活(等输入/验证码/点击)。机制原因写在描述里:子任务结束、
  被 cancel/kill/deadline 收尾、或桥接层退出时都会**连进程树一起收尾**,服务会跟着消失,
  "某个进程还活着"也写不成可机检的 acceptance —— 要常驻的东西由调用方自己起后台作业。
  一次性短命服务(起测试库、用完就停)仍可派,但必须成对收尾,交付物不能是"进程还活着"。
