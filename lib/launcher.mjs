...
/**
 * 校验垫片里的入口文件是否还在;不在就按候选清单自愈改指,全都不是则响亮报错。
 *
 ...
 * 这里把它收敛成一处:能自愈就自愈,不能自愈就报一条人能看懂的错。
 *
 * @param shim - 垫片路径(只用于报错)。
 * @param command - 垫片里的可执行文件;`dirname` 即安装根。
 * @param args - 垫片里的固定参数。
 * @returns 入口修好后的参数数组(顺序不变)。
 */
...
export function resolveLauncher() {
	...
	for (const candidate of candidates) {
		...
		// 入口文件不存在就自愈;自愈不可能时**响亮报错**(不静默降级,也不谎称成功)
		const args = repairEntryPoint(candidate, parsed.command, parsed.fixedArgs);
		return {
