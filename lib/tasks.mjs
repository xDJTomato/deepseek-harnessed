...
/**
 * 定位子进程会话日志(session.jsonl.zstd)。stderr.log 自带推理流,
 * 会话日志是"真的写进了事件流"的第二重证据;找不到就安静返回 null。
 * @param record - 任务记录(需要 workspace 与 sessionId)。
...
function sessionLogPath(record) {
	...
	const workspace = typeof record.workspace === 'string' && record.workspace !== '' ? record.workspace : '';
	if (sessionId === '' || workspace === '') return null;
	const candidate = join(dshHome(), 'sessions', workspaceKey(workspace), sessionId, 'session.jsonl.zstd');
	return fileBytes(candidate) > 0 ? candidate : null;
}
...
export async function startTask(options) {
	...
	const done = new Promise((settle) => {
		...
		handle.finish = (status, extra = {}) => {
			...
			const result = readText(resultPath) || readText(stdoutPath);
