...
function delegationPreamble({ client, workspace, raw }) {
	...
	return [
		...
		'- 这次任务是从更大的目标里拆出来的一小片:只做这一片、按验收标准收工,不要顺手扩大范围;发现还需要更大的改动时,只在最终汇报里说明,不要自行开工。',
...
export const DELEGATION_INSTRUCTIONS = [
	...
	'3) 先让它自己等,再决定要不要轮询:dsh_task 默认阻塞 ' + DEFAULT_WAIT_SECONDS + 's(短超时),窗口内结束则结果**由这次调用直接带回**,不用轮询;只有返回 status="running" 才转 dsh_task_status(job_id=..., wait_seconds=' + DEFAULT_STATUS_WAIT_SECONDS + ') 一轮一轮推到终态。别传 wait_seconds=0(一次调用变十几次轮询,每轮都是你完整一轮推理);也别调过 60s(多数 harness 的单次工具超时就 60s,会先被掐断)。',
	'4) 状态语义:ok 完成(仍需按 acceptance 复核);error 看 stderr.log 尾部后小步重试;deadline 说明你估小了(拆小或把秒数提到 1.5~2 倍);stalled 是日志零增长且进程树全静止、已被杀(看 recent_activity 找卡点再重派);killed 被 dsh_task_kill 停;timeout 外层兜底到顶。',
	'5) 止损:dsh_task_cancel 优雅停,dsh_task_kill 强杀整棵进程树(按 job_id 或按 caller 停掉自己起的全部任务);跑偏了、久无进展就停,别耗着。',
	'6) 排查:recent_activity / progress_bytes / last_progress_at,现场 $DSH_HOME/subagent/state/tasks/<job_id>/{result.txt,meta.json,stderr.log}。',
	'7) 禁止:① 不要把超过 20 分钟的任务塞进单次 dsh_task;② permission 收窄时不要要求子代理用 pwsh 写文件(受限档位下返回全空),写文件用 write/edit;③ 短指令作业(一条命令 / 改一行 / 只读查询)自己做完,不要派;常驻进程(dev server、watch、daemon、交互式终端、需要成对收尾的独占资源)不要派 —— 会随任务结束被连树收尾,要常驻就调用方自己起后台作业。',
	'8) 先拆再并行:一次派活 = 一个可独立验收的产物,expected_seconds ≤ 300s;超 300s 或涉 3 个以上文件就先拆。互不依赖的子任务**同批并发派**(别串行等一个再派下一个),有依赖的才串行。',
	'',
	'监控:dsh_task 会自动确保有监控窗口在跑(已有宿主绝不重复启动);返回值 monitor_host:already-running / launched / skipped / unavailable。',
].join('\n');
	...
	return [
		{
			...
			description: [
				...
				'',
				'【先拆再派,能并行就并行】一次 dsh_task 只派**一个可独立验收的产物**(一个文件 / 一个模块 / 一条链路),目标 `expected_seconds` ≤ 300s。要碰 3 个以上文件、估时超过 300s、或本身就是"先调研再实现再验证"这种多阶段作业 —— **先拆成多个小任务再派**;大重构那类 900~1800s 的活一律拆成阶段性任务(每段各自可验收),不要一次塞进去赌一把。拆完按依赖关系决定顺序:互不依赖的子任务**同一轮里并发派出**(同时起多次 dsh_task;当前在跑数看 dsh_health 的 activeByCaller),有依赖的才串行。并行的每个子任务仍要有自包含 prompt 与可机检 acceptance,产物由你合并复核。',
				'【这些活不要派给子代理】① **短指令作业**:一条 shell 命令、一次只读查询、改一行配置或文案、看一个文件、跑一次 `git status` —— 顺手就能做完的,你自己做,派出去反而更慢(还要起一个完整代理进程)。② **常驻 / 长命进程**:dev server(`uvicorn`、`pnpm dev`)、watch、daemon、交互式终端、需要成对收尾的独占资源(占端口、开迁移、拿锁)—— 这类**禁止**派:子任务结束、被 cancel/kill/deadline 收尾、或桥接层退出时都会**连进程树一起收尾**,服务会跟着消失;而且"某个进程还活着"也没法作为可机检的 acceptance。要常驻的东西由**你自己**起后台作业,并把地址、端口与停止方式写进你的回复。③ 需要人交互的活(等输入、等验证码、等点击)同样不派 —— 子代理无人应答。',
				...
				STATUS_SEMANTICS,
				FAILURE_TRIAGE,
				'【一句话禁止项】① 不要把超过 20 分钟的任务塞进单次 dsh_task —— 拆开派;也不要为了省事把本来互不依赖的子任务串起来一个个等 —— 那是并发派;② permission 收窄成 read-only/workspace-write 时,不要要求子代理用 pwsh 写文件(受限档位下 pwsh 在 Windows 上返回全空),要它写文件就用 write/edit 文件工具。',
			].join('\n'),
			inputSchema: {
				type: 'object',
				properties: {
