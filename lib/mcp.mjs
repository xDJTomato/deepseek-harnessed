 ...
 *   dsh_task_status 轮询/等待某个 DSH 任务(含 recent_activity 活动流,便于每 20~30s 看一眼)
...
import { BRIDGE_ROOT, clip, dshHome, readText } from './util.mjs';
...
/**
 * 默认等待秒数:dsh_task 在此时间内完成就直接返回结果,否则返回 job_id。
 * 刻意压到 30s:让调用方每 20~30s 主动看一眼进度,而不是一次阻塞两分钟。
 */
const DEFAULT_WAIT_SECONDS = Number(process.env.DSH_SUBAGENT_WAIT_SECONDS ?? 30);
...
export const DELEGATION_INSTRUCTIONS = [
	...
	'3) 不要阻塞:wait_seconds 默认 ' + DEFAULT_WAIT_SECONDS + ',建议 0 或 ≤30;拿到 job_id 就每 20~30 秒用 dsh_task_status(wait_seconds=25) 轮询,同一轮里推进,绝不在任务还在跑时结束回答或臆测结果。',
	...
	return [
		{
			...
			description: [
				...
				'DSH 会在你指定的工作空间里以**独立代理进程**运行:独立上下文窗口、独立工具(文件读写/编辑、shell、联网搜索、子代理)和独立模型(默认 deepseek-v4.1-flash)。它真的会动手干活:改文件、跑构建、跑测试。',
				...
				'【不要阻塞,轮询推进】`wait_seconds` 默认 ' + DEFAULT_WAIT_SECONDS + ';建议传 0(拿到 job_id 立刻走)或 ≤30。若返回 status="running",就在**同一轮**里用 dsh_task_status(job_id=..., wait_seconds=25) 每 20~30 秒看一次,直到出现终态。不要一次等 10 分钟,更不要在任务还在跑的时候结束你的回答。',
				STATUS_SEMANTICS,
				FAILURE_TRIAGE,
			...
			inputSchema: {
				...
				properties: {
					...
					wait_seconds: {
						type: 'number',
						description: `本次调用最多阻塞多久等结果;默认 ${DEFAULT_WAIT_SECONDS},建议 0(立刻拿 job_id)或 ≤30。超时不算失败:会返回 status="running" 和 job_id,你用 dsh_task_status 每 20~30 秒轮询一次即可。不要传很大的值来"一次等完"。`,
					},
					timeout_seconds: {
						type: 'number',
					...
					model: {
						type: 'string',
						description: '本次运行使用的 DSH 模型 id(如 "deepseek-v4-pro"、"claude-sonnet-4.6")。默认用 DSH 配置的默认模型。简单任务不要为了省钱换小模型而制造返工。',
					},
					provider: {
						type: 'string',
						description: '本次运行使用的模型服务商 id(例如 "deepseek-official";以你 DSH 里的 provider 名为准)。一般省略,交给 DSH 默认配置。',
					},
					permission: {
						type: 'string',
		...
		},
		{
			...
			description: [
				'查看(并可选等待)某个 dsh_task 任务的状态。任务运行期间**每 20~30 秒**调一次(wait_seconds=25),直到出现终态。',
			...
			inputSchema: {
				...
				properties: {
					...
					wait_seconds: { type: 'number', description: '本次最多阻塞多久等它变终态。默认 0(立即返回现状);推荐 20~30,配合每 20~30 秒一次的轮询节奏。' },
				},
				required: ['job_id'],
			},
		},
		...
		},
		{
			...
			description: [
				...
				'什么时候用:① 开局自检("这个 harness 到底连上没有");② 任务好像没动,先看 liveTasks 的 progressBytes/silenceSeconds;③ 想确认自己起的任务有哪些,再决定 kill 谁。',
			].join('\n'),
...
export class McpServer {
	...
	handleNotification(method, params) {
		...
		if (method === 'notifications/cancelled') {
			// 客户端放弃了一次工具调用:留给任务自己按 timeout 收尾,不误杀其他任务。
			void params;
		}
	}
	...
	async callTool(name, args) {
		try {
			switch (name) {
				...
				case 'dsh_task':
					return this.wrap(await this.runTask(args));
				case 'dsh_task_status':
					return this.wrap(await this.statusText(args));
	...
	async statusText(args) {
		const id = String(args.job_id ?? '');
		if (id === '') return this.fail('job_id 不能为空。');
		const waitSeconds = Number.isFinite(Number(args.wait_seconds)) ? Number(args.wait_seconds) : 0;
	...
	/** 把任务状态渲染成给模型看的文本。 */
	renderTask(state, { waitedSeconds, kind, monitorHost }) {
		...
		if (state.status === 'running') {
			...
			return [
				...
				'',
				`提示词已交给 DSH 子代理执行,但 ${waitedSeconds}s 内尚未结束。`,
				`请在同一轮内继续轮询:dsh_task_status(job_id="${state.id}", wait_seconds=25),大约每 20~30s 一次,直到 status 变为 ok / error / deadline / stalled / cancelled / killed。不要在此刻结束回答,也不要臆测结果。`,
