	...
	return [
		{
			...
			inputSchema: {
				...
				properties: {
					...
					model: {
						type: 'string',
						description: '本次运行使用的 DSH 模型 id —— 取值必须是你本机 DSH 里**已配置**的模型:$DSH_HOME/settings.yaml 里 llm-pi-ai.providers.<provider>.models[].id 或 llm-deepseek.models[](本机当前是 "deepseek-v4.1-flash")。默认用 DSH 配置的默认模型。写错的 id **不会回退到默认模型**:DSH 在请求发出前就直接失败(UNKNOWN_MODEL)。简单任务不要为了省钱换小模型而制造返工。',
					},
					provider: {
						type: 'string',
						description: '本次运行使用的模型服务商 id —— 取值必须是你本机 DSH 里**已注册**的 provider(即 settings.yaml 里 providers 下的键;本机当前是 "rigol")。一般省略,交给 DSH 默认配置。写错的 id **不会回退**:DSH 直接失败(NO_ADAPTER)。',
					},
					reasoning_effort: {
						type: 'string',
